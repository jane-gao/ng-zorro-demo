...
export class AnnounceEditComponent implements OnInit {
  ...
  submitAnnounceForm() {
    ...
    if (formValue.state) formValue.state = Setting.ENUMSTATE.announceState.online;
    else formValue.state = Setting.ENUMSTATE.announceState.noline;
    if (formValue.isTop) formValue.isTop = Setting.ENUMSTATE.yes;
    else formValue.isTop = Setting.ENUMSTATE.no;
    switch (me.path) {
      case "add":  //创建公告
